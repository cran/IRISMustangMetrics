...
crossCorrelationMetrics <- function(st1, st2, maxLagSecs=10, filter=signal::butter(2,0.2)) {
  ...
  if (length(st1@traces) > 1) {
    stop(paste("crossCorrelationMetrics:",st1@traces[[1]]@id,"has multiple traces."))
  }
  if (length(st2@traces) > 1) {
    stop(paste("crossCorrelationMetrics:",st2@traces[[1]]@id,"has multiple traces."))
  }
  ...
  if (tr1@stats@sampling_rate < 1) {
    stop(paste("crossCorrelationMetrics:",tr1@id,"has a sampling_rate < 1."))
  }
  if (tr2@stats@sampling_rate < 1) {
    stop(paste("crossCorrelationMetrics:",tr2@id,"has a sampling_rate < 1."))
  }
  ...
  if (sr1 > sampling_rate) {
    if (pracma::rem(sr1,sampling_rate) > 0) {
      stop(paste0("crossCorrelationMetrics: sampling rates are not multiples of eachother:",
  ...
  if ( any(is.na(d1)) || any(is.na(d2)) ) {
    stop(paste("crossCorrelationMetrics: NA values generated during resampling"))
  }
