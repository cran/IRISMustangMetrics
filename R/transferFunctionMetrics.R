...
transferFunctionMetrics <- function(st1, st2) {
  ...
  if (length(st1@traces) > 1) {
    stop(paste("transferFunctionMetrics:",st1@traces[[1]]@id,"has multiple traces."))
  }
  if (length(st2@traces) > 1) {
    stop(paste("transferFunctionMetrics:",st2@traces[[1]]@id,"has multiple traces."))
  }
  ...
  if (difftime(tr2@stats@starttime,starttime,units='sec') > 1) {
    stop(paste("transferFunctionMetrics: starttimes don't match:",tr2@stats@starttime,",",starttime))
  }
  if (difftime(tr2@stats@endtime,endtime,units='sec') > 1) {
    stop(paste("transferFunctionMetrics: endtimes don't match:",tr2@stats@endtime,",",endtime))
  }
  
  # Sanity check sampling rates
  if (tr1@stats@sampling_rate < 1) {
    stop(paste("transferFunctionMetrics:",tr1@id,"has a sampling_rate < 1."))
  }
  if (tr2@stats@sampling_rate < 1) {
    stop(paste("transferFunctionMetrics:",tr2@id,"has a sampling_rate < 1."))
  } 
  ...
  if (sr1 > sampling_rate) {
    if (pracma::rem(sr1,sampling_rate) > 0) {
      stop(paste("transferFunctionMetrics: sampling rates are not multiples of eachother"))
    }
  ...
  if ( any(is.na(d1)) || any(is.na(d2)) ) {
    stop(paste("transferFunctionMetrics: NA values generated during resampling"))
  }
